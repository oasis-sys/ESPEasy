...
P200_Task::P200_Task(taskIndex_t taskIndex) : _taskIndex(taskIndex) {
  serial_buffer.reserve(P200_DATAGRAM_MAX_SIZE);
}
...
bool P200_Task::serverActive(WiFiServer *server) {
# if defined(ESP8266)
...
void P200_Task::startServer(uint16_t portnumber) {
...
bool P200_Task::hasClientConnected() {
...
void P200_Task::discardClientIn() {
...
void P200_Task::clearBuffer() {
  serial_buffer = String();
  serial_buffer.reserve(P200_DATAGRAM_MAX_SIZE);
}

void P200_Task::serialBegin(const ESPEasySerialPort port, int16_t rxPin, int16_t txPin, unsigned long baud, uint8_t config) {
  serialEnd();
...
void P200_Task::handleClientIn(struct EventStruct *event) {
  int count      = ser2netClient.available();
  int bytes_read = 0;
  uint8_t net_buf[P200_DATAGRAM_MAX_SIZE];

  if (count > 0) {
    if (count > P200_DATAGRAM_MAX_SIZE) { count = P200_DATAGRAM_MAX_SIZE; }
...
void P200_Task::handleSerialIn(struct EventStruct *event) {
  if (nullptr == ser2netSerial) { return; }
  int RXWait  = P200_RX_WAIT;
  int timeOut = RXWait;

  do {
    if (ser2netSerial->available()) {
      if (serial_buffer.length() > static_cast<size_t>(P200_RX_BUFFER)) {
        # ifndef BUILD_NO_DEBUG
...
void P200_Task::discardSerialIn() {
  if (nullptr != ser2netSerial) {
...
void P200_Task::rulesEngine(const String& message) {
...
bool P200_Task::isInit() const {
  return nullptr != ser2netServer && nullptr != ser2netSerial;
}

void P200_Task::sendConnectedEvent(bool connected)
{
